package net.calca.biomesofcataclysms.data.server;
...
import static net.calca.biomesofcataclysms.data.server.DataSavingHelper.*;
import static net.calca.biomesofcataclysms.management.server.chunk.flood.FloodProcessorHelper.resetFloodWaves;
...
public class PersistentData {
    ...
    public record DedicatedSyncMessage(String cataclysm, int state, Set<String> deletedBiomes) implements CustomPacketPayload {
        ...
        public static void handleData(final DedicatedSyncMessage message, final IPayloadContext context) {
            ...
            context.enqueueWork(() -> {
                // Assegnazione ai nuovi endpoint del ClientDataAccessPoint
                ClientDataAccessPoint.clientData.mapData.cataclysm = message.cataclysm;
                ClientDataAccessPoint.clientData.mapData.state = message.state;

                // Sincronizzazione dei biomi cancellati sul client
                ClientDataAccessPoint.clientData.mapData.deletedBiomes.clear();
                ClientDataAccessPoint.clientData.mapData.deletedBiomes.addAll(message.deletedBiomes);

                /* Nota: Nel tuo codice originale avevi:
                 * MapVariables.clientSide.biomesAffected = message.deletedBiomes.size();
                 * Se questa variabile ti serve ancora, ricordati di aggiungerla alla tua classe
                 * ClientMapData e di popolarla qui. Ad esempio:
                 * ClientDataAccessPoint.clientData.mapData.biomesAffected = message.deletedBiomes.size();
                 */
    ...
    public record BloodMoonEventsSyncMessage(Map<String, Integer> eternalEclipseBloodMoonEvents) implements CustomPacketPayload {
        ...
        public static void handleData(final BloodMoonEventsSyncMessage message, final IPayloadContext context) {
            ...
            context.enqueueWork(() -> {
                // Controllo di sicurezza: inizializza la mappa nel ClientDataAccessPoint se dovesse essere null
                if (ClientDataAccessPoint.clientData.mapData.eternalEclipseBloodMoonEvents == null) {
                    ClientDataAccessPoint.clientData.mapData.eternalEclipseBloodMoonEvents = new HashMap<>();
                }

                // Svuota e aggiorna la mappa con i dati ricevuti dal server
                ClientDataAccessPoint.clientData.mapData.eternalEclipseBloodMoonEvents.clear();
                if (message.eternalEclipseBloodMoonEvents != null) {
                    ClientDataAccessPoint.clientData.mapData.eternalEclipseBloodMoonEvents.putAll(message.eternalEclipseBloodMoonEvents);
                }

                // Se usi un flag di aggiornamento sul cielo, ricordati di puntare correttamente all'istanza di clientData
                ClientDataAccessPoint.clientData.skyData.isDirty = true;
    ...
    public record RealServerTimeSyncMessage(long rawServerTime, int vanillaMoonPhase) implements CustomPacketPayload {
        ...
        public static void handleData(final RealServerTimeSyncMessage message, final IPayloadContext context) {
            ...
            context.enqueueWork(() -> {
                ...
                clientData.skyData.serverDayTime = message.rawServerTime;
