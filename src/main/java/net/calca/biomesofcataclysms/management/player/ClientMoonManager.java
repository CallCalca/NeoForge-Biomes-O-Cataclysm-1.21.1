...
public class ClientMoonManager {
    ...
    public static void tick(ClientTickEvent.Post event){
        ...
        if (!currentBiomeId.equals(lastBiomeId) || MoonController.isDirty) {
            MoonController.isDirty = false; // Resettiamo il flag

            // Eseguiamo il calcolo una volta sola per questo bioma!
            MoonController.reset();
            MoonController.updateBloodMoonEvents(level, pos);
        }

        // 1. Verifichiamo se il bioma è cambiato o se i dati sono "dirty"
        boolean biomeChanged = !currentBiomeId.equals(lastBiomeId);

        if (biomeChanged || MoonController.isDirty) {
            MoonController.isDirty = false; // Resettiamo il flag
        ...
        boolean isCurrentBiomeDeleted = globalVars.deletedBiomes.contains(ModUtils.getBiomeID(level, pos));
        ...
        if (isCurrentBiomeDeleted && (biomeChanged || bloodMoonEvents != MoonController.bloodMoonEvents)) {
    ...
    public static void startBiomeEvent(ClientLevel level, BlockPos playerPos) {
        PersistentData.MapVariables globalVars = PersistentData.MapVariables.get(level);
        int moonPhase = globalVars.eternalEclipseBloodMoonEvents.getOrDefault(ModUtils.getBiomeID(level, playerPos), 0);
